...
export const TestimonalApi = createApi({
  ...
  baseQuery: fetchBaseQuery({
    baseUrl: import.meta.env.VITE_API_URL + "/testimonal",
  ...
  endpoints: (builder) => ({
    listAll: builder.query({
        ...
        providesTags: (result) => 
          result?.result
            ? [
                ...result.result.map(({ _id }) => ({ type: 'Testimonal', id: _id })),
                { type: 'Testimonal', id: 'LIST' },
              ]
            : [{ type: 'Testimonal', id: 'LIST' }],
    ...
    showById: builder.query({
      query: (id) => `/${id}`,
      providesTags: (result, _err, id) => [{ type: "Testimonal", id }],
    }),
    ...
    create: builder.mutation({
      ...
      invalidatesTags: [{ type: "Testimonal", id: "LIST" }],
    }),
